...
Stepper::Stepper(int mstr_channel, int accel_size)
{
	...
	enable_channel = SM_ENABLE_CHANNEL;
};
...
void Stepper::Init(int mode, unsigned long start, unsigned long slew)
{
	...
	/************** Half-Step Mode ***************/
	else if(my_output_mode == ECE315_ETPU_SM_HALF_STEP_MODE){
		//Define steps per revolution for HALF STEP
		printf("IN");
		steps_per_rev = STEPS_PER_REV_HALF_STEP;
		if (fs_etpu_sm_init(
			master_channel,				// Stepper Stepper master channel
			ECE315_ETPU_SM_2PHASE_HALF_STEP_DRIVE,  // Use two phase full step configuration
			0,						// Motor Start Position
			start,					// Start Period
			slew,					// Slew Period
			my_accel_tbl, 			// Pointer to acceleration table
		...
		if (fs_etpu_sm_init(
			enable_channel,				// Stepper Stepper master channel
			ECE315_ETPU_SM_2PHASE_HALF_STEP_ENA  ,  // Use two phase full step configuration
			 0,						// Motor Start Position
			 start,					// Start Period
		     slew,					// Slew Period
			my_accel_tbl, 			// Pointer to acceleration table
		...
		fs_etpu_sm_enable(enable_channel, FS_ETPU_PRIORITY_LOW);
	}
	delete 	my_accel_tbl;
}
...
void Stepper::NewAccelTable()
{
	...
	if (my_output_mode == ECE315_ETPU_SM_HALF_STEP_MODE ){
		fs_etpu_sm_table(enable_channel, my_accel_tbl);
	}
	delete 	my_accel_tbl;
}
...
void Stepper::Step(int steps)
{
	...
	if (my_output_mode == ECE315_ETPU_SM_HALF_STEP_MODE ){
		fs_etpu_sm_set_dp(enable_channel, fs_etpu_sm_get_cp(enable_channel) + steps);
	}
}
...
unsigned int Stepper::SetSlewPeriod(unsigned int slew)
{
	...
	if (my_output_mode == ECE315_ETPU_SM_HALF_STEP_MODE ){
		fs_etpu_sm_set_sp(enable_channel, slew);
	}
	NewAccelTable();
...
unsigned int Stepper::SetStartPeriod(unsigned int start)
{
	...
	if (my_output_mode == ECE315_ETPU_SM_HALF_STEP_MODE ){
		fs_etpu_sm_set_st(enable_channel, start);
	}
...
void Stepper::Stop(void ) {
	...
	fs_etpu_sm_enable( master_channel, FS_ETPU_PRIORITY_LOW);
	if (my_output_mode == ECE315_ETPU_SM_HALF_STEP_MODE ){
		fs_etpu_sm_disable(enable_channel, FS_ETPU_SM_DISABLE_LOW);
		fs_etpu_sm_enable(enable_channel, FS_ETPU_PRIORITY_LOW);
	}
}
